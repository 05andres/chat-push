...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">Chat global</div>
                <ol class="chat">

                </ol>

                <div class="card-body" id="{{ auth()->user()->id }}" name="{{ Auth::user()->name }}">
                    @if (session('status'))
                    ...
                    @endif
                        ...
                        <p>chatea: <input type="text" id="test" value=""></p>
            ...
            <p style="font-weight: bold;">MIS AMIGOS:</p>
            @foreach($onlines as $online)
                @if($online->isOnline())
                    <a href="{{ route('privado',$online->id)}}"><p> {{$online->name}} <p/></a>
                @else
                    <p> {{$online->name}} <p/>
                @endif

            @endforeach
            <p style="font-weight: bold;">Sugerencias de amigos:</p>
            @foreach($todos as $user)
                @if(auth()->user()->amigos($user->id)==true)
                    @if($user->id  != auth()->user()->id)
                       <div><span class="space" >{{$user->name}}</span><a href="{{ route('agregar',$user->id)}}"> agregar amigo</a></div>
                    @endif
                @endif
            @endforeach
        </div>
...
$(document).ready(function(){
    ...
    var userId=$(".card-body").attr('id');
    var name=$(".card-body").attr('name');
    console.log(userId);
    ...
    $("button").click(function(){
        ...
        m=checkTime(m);
        var hora=h+":"+m;
        var mensaje= name+" dice:"+$("#test").val();
        $("#test").val("");
        ...
        $( ".chat" ).append(html);
        conn.send(JSON.stringify({command: "groupchat", message: mensaje, channel: "global"}));
